...
func buildCNIConfig(netCfg *vmv1alpha1.NetworkConfig) (string, error) {
    cfg := baseConfig{
        ...
        DisableContainerInterface: true,     // KubeVirt 需要，直接将桥接连接到 VM
    }
    ...
    if netCfg.IPConfig != nil {
        if netCfg.IPConfig.Mode == "dhcp" {
            ...
            // 使用 disableContainerInterface: true 时，IP 在 VM 内部通过 Cloud-Init DHCP 获取
        } else if netCfg.IPConfig.Mode == "static" && netCfg.IPConfig.Address != nil {
            // bridge CNI 使用 host-local IPAM 配置静态 IP
            ip, ipNet, err := net.ParseCIDR(*netCfg.IPConfig.Address)
            if err != nil {
                return "", err
            }
...
**注意**：DHCP 模式不设置 IPAM。IP 在 VM 内部通过 Cloud-Init DHCP 获取。
