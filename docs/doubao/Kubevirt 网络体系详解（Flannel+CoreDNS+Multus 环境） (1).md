...
- **辅助网卡（接入物理业务网）**：
    ...
    - **IP 获取方式：支持静态指定和动态 DHCP 获取两种模式**动态获取（DHCP）：依赖业务网中的 DHCP 服务器，通过配置 Multus 网络启用 DHCP 客户端，虚拟机启动后自动从 DHCP 服务器获取 IP、子网掩码、网关、DNS 等网络配置。

    - 静态指定：若业务网无 DHCP 服务器或需固定 IP，可在创建虚拟机时手动指定静态 IP，需确保 IP 未被占用且与业务网网段匹配。

    #### （3）业务网卡 IP 配置实操：静态指定与 DHCP 客户端配置

    以下是两种 IP 获取方式的具体配置步骤，基于 Macvlan 辅助网络（SR-IOV 配置逻辑类似，仅 CNI 插件参数差异）：

    ##### ① 动态获取：配置 DHCP 客户端

    核心逻辑：通过 NetworkAttachmentDefinition 定义支持 DHCP 的 Macvlan 网络，Multus 会自动为虚拟机辅助网卡启动 DHCP 客户端。

    1. 创建支持 DHCP 的 Macvlan 网络（NetworkAttachmentDefinition）：
            `apiVersion: k8s.cni.cncf.io/v1
    kind: NetworkAttachmentDefinition
    metadata:
      name: macvlan-dhcp-network  # 网络名称，后续创建虚拟机时引用
      namespace: default
    spec:
      config: |-
        {
          "cniVersion": "0.3.1",
          "type": "macvlan",
          "master": "eth1",  # 关联宿主机业务网卡 eth1
          "mode": "bridge",  # Macvlan 模式，bridge 模式支持同一网段设备互通
          "ipam": {
            "type": "dhcp"  # 启用 DHCP 客户端，自动获取 IP
          }
    ` `    }`

    2. 创建虚拟机时关联该 DHCP 网络（VMI 配置片段）：
        ...
        devices:
          interfaces:
            ...
            - name: flannel-net
              bridge: {}
            # 辅助网卡（DHCP 动态获取 IP）
            - name: macvlan-dhcp-net
              bridge: {}
      networks:
        # 关联 Flannel 主网络（需提前定义）
        - name: flannel-net
          pod: {}
        # 关联上述创建的 DHCP 辅助网络
        - name: macvlan-dhcp-net
          multus:
    ` `        networkName: default/macvlan-dhcp-network`

    3. 验证：虚拟机启动后，通过 `virtctl console vm-dhcp-demo` 登录，执行 `ip addr` 可查看辅助网卡已自动获取 DHCP 分配的 IP。

    ##### ② 静态指定：手动配置固定 IP

    核心逻辑：在 VMI 配置中直接指定辅助网卡的静态 IP，无需依赖 DHCP 服务器。

    1. 创建 Macvlan 网络（NetworkAttachmentDefinition，无需 DHCP 配置）：
            `apiVersion: k8s.cni.cncf.io/v1
    kind: NetworkAttachmentDefinition
    metadata:
      name: macvlan-static-network
      namespace: default
    spec:
      config: |-
        {
          ...
          "mode": "bridge",
          "ipam": {
            "type": "host-local"  # 本地 IP 管理，支持静态指定
          }
    ` `    }`

    2. 创建虚拟机时指定静态 IP（VMI 配置片段）：
            `apiVersion: kubevirt.io/v1
    kind: VirtualMachineInstance
    metadata:
      name: vm-static-demo
    spec:
      ...
      networks:
        ...
        - name: macvlan-static-net
          multus:
            networkName: default/macvlan-static-network
            defaultGateway: true  # 可选，设为默认网关（业务网优先）
          ipAddress: "10.0.0.50/24"  # 手动指定静态 IP，需与业务网网段匹配
          gateway: "10.0.0.1"  # 业务网网关
          dnsConfig:
            nameservers:
    ` `          - "10.0.0.2"  # 业务网 DNS 服务器`

    3. 验证：登录虚拟机后执行 `ip addr`，辅助网卡 IP 应为指定的 10.0.0.50/24；执行`ping 10.0.0.1` 可测试网关连通性。

    配置注意事项：1. 启用 DHCP 时，需确保业务网（宿主机 eth1 网段）存在可用的 DHCP 服务器，且未禁止虚拟机网段的 IP 分配；2. 静态指定 IP 时，需提前规划 IP 地址池，避免与其他设备 IP 冲突；3. 若同时配置主网卡和辅助网卡的网关，需在虚拟机内手动调整路由优先级，避免网络访问异常。
