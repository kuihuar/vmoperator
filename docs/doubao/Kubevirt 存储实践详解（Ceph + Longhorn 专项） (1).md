...
|操作类型|存储类型|核心原理|依赖技术|关键特性/注意事项|
|---|---|---|---|---|
|导入|Ceph|从 RGW 对象存储读取系统盘/数据盘镜像数据，通过 CDI 组件调用 Ceph CSI 驱动，还原为 RBD 块存储卷|Ceph RGW（S3 兼容接口）、Kubevirt CDI、Ceph CSI 驱动、RBD 镜像格式转换|需分别导入双盘镜像，确保容量与原盘一致；跨集群导入需同步 RGW 镜像并验证网络连通性|
||Longhorn|从 S3 兼容对象存储读取系统盘/数据盘备份数据，通过 CDI 组件调用 Longhorn CSI 驱动，基于快照还原为 Longhorn 卷|Longhorn 卷备份机制、S3 协议、Kubevirt CDI、Longhorn CSI 驱动|双盘备份需关联标签便于匹配；导入后卷副本自动跨节点分布，提升高可用|
|导出|Ceph|通过 CDI 组件调用 Ceph CSI 驱动，将系统盘/数据盘的 RBD 卷数据同步至 RGW 对象存储，生成镜像文件|Ceph RGW、Kubevirt CDI、Ceph CSI 驱动、RBD 块存储|双盘需分别创建 DataVolume 导出；建议统一存储路径；离线导出更安全，在线导出需启用 qemu-guest-agent|
||Longhorn|先对系统盘/数据盘卷创建快照，基于快照生成增量备份数据，通过 S3 协议同步至对象存储|Longhorn 快照机制、增量备份技术、S3 协议、Longhorn CSI 驱动|配置统一 S3 备份目标；双盘备份添加相同标签关联；通过 Longhorn UI 可直观查看备份状态|
|迁移|Ceph|依托 RBD 共享存储特性，无需拷贝双盘块数据，仅同步虚拟机内存状态与设备配置，目标节点直接挂载 RBD 卷完成切换|Kubevirt 动态迁移框架（libvirt/QEMU 协议）、Ceph RBD 共享存储、CSI 卷挂载协议、qemu-guest-agent|双盘同步迁移，业务中断秒级；迁移前冻结文件系统保障数据一致；可指定迁移带宽避免占用过多存储网络|
||Longhorn|利用卷多副本跨节点分布特性，迁移时优先使用目标节点本地/近节点副本，同步虚拟机状态后完成双盘挂载切换|Longhorn 跨节点副本同步（Raft 协议）、Kubevirt 动态迁移框架、CSI 卷迁移协议、qemu-guest-agent|双盘迁移原子性，单盘失败则回滚；副本同步优先，无副本时触发快速同步；迁移后验证副本分布与数据完整性|
## 五、总结
