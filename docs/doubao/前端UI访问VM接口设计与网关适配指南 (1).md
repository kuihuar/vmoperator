...
为规范前后端错误处理逻辑，统一错误码格式与含义，以下梳理核心接口涉及的错误码及通用错误码，前端可根据错误码类型快速定位问题并给出友好提示。

|错误码|错误类型|错误描述|适用场景|前端处理建议|
|---|---|---|---|---|
|200|成功|操作执行成功|所有接口成功响应|正常处理返回数据|
|40001|参数错误|请求参数缺失或格式不正确|登录接口（用户名/密码为空）、创建Service接口（端口格式错误）等|提示用户“参数错误，请检查输入后重试”，标注错误参数项|
|40101|认证失败|用户名或密码错误|登录接口|提示用户“用户名或密码错误，请重新输入”|
|40102|Token无效/过期|访问Token不存在、已过期或验证失败|所有需认证的接口（VM列表查询、控制台访问等）|提示用户“登录已过期，请重新登录”，跳转至登录页|
|40301|权限不足|无对应操作权限|权限校验接口、VM启停操作、控制台访问等|提示用户“无操作权限，请联系管理员申请”，禁用对应操作按钮|
|40401|资源不存在|请求的VM、Service、任务等资源不存在|VM详情查询、任务状态查询、删除不存在的Service等|提示用户“请求的资源不存在”，刷新资源列表|
|40901|资源冲突|资源状态不允许当前操作（如重复创建同名Service、操作已停止的VM）|创建Service接口、VM启停操作等|提示用户“资源状态冲突，请检查资源状态后重试”|
|50001|服务器内部错误|后端服务异常|所有接口|提示用户“服务异常，请稍后重试”，记录错误日志便于排查|
|50301|服务不可用|后端服务暂时不可用（如Kubernetes集群连接失败）|所有接口|提示用户“服务暂时不可用，请稍后重试”，定时重试接口|
---
