...
如果您遇到多个问题同时出现（如 Multus 崩溃 + Longhorn Manager 重启 + driver-deployer 卡住），请先运行综合诊断：
...
**常见组合问题**：
- Multus Pod 崩溃 + Longhorn Manager 重启 → 通常是 DNS 问题 + Multus CNI 路径问题
- Manager 重启 + driver-deployer Init → driver-deployer 在等待 Manager 就绪
